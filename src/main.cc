...
int main() {
  ...
  Resonance resonance[kNumRes];
  for (int i = 0; i < kNumRes; i++) resonance[i].SetResonanceParameters(IDs[i]);
  ...
  for (int i = 0; i < kNumPts; i++) {
    ...
    for (int j = 0; j < kNumRes; j++) {
      TComplex EE = resonance[j].Multipole(xx[i], true);
      yy[j][0][i] = EE.Re();
      yy[j][1][i] = EE.Im();
      TComplex MM = resonance[j].Multipole(xx[i], false);
      yy[j][2][i] = MM.Re();
      yy[j][3][i] = MM.Im();
    }
  }
